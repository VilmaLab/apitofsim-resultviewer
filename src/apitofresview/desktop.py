"""Running the result viewer as a desktop application.
...
# The active pywebview Window, or None when running browser-backed.
_window = None
...
def run_window(app, sock, title="APiToF Result Viewer", log_level="warning"):
    global _window
    # pywebview is not installed on Linux (see module docstring); this path is
    # only reached on Windows and macOS.
    import webview  # type: ignore[reportMissingImports]

    server = ServerThread(app, sock, log_level=log_level).start()
    _window = webview.create_window(title, server.url, width=1400, height=900)
    try:
...
def run_browser(app, sock, open_browser=True, log_level="info"):
    server = ServerThread(app, sock, log_level=log_level).start()
    print(f"APiToF Result Viewer running at {server.url}  (Ctrl-C to quit)", flush=True)
    if open_browser:
